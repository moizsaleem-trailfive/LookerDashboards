...
view: events_luba {
  ...
  measure: sollitatie {
    type: count_distinct
    sql: CASE
          WHEN (${utm_id_integer} IS NOT NULL OR ${traffic_source__name} IS NOT NULL  OR ${traffic_source__medium}  IS NOT NULL  OR ${traffic_source__source} IS NOT NULL)AND ${session_id} is not null AND ${user_pseudo_id} is not null
