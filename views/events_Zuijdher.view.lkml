view: events_Zuijdher {
  sql_table_name: `evident-catcher-381918.analytics_310277048.events_*` ;;
  ...
  dimension: Page_location{
    ...
    sql: (SELECT value.string_value
             ...
             WHERE event_name="sollicitatie_ontvangen2" AND key = 'page_referrer' AND REGEXP_EXTRACT(value.string_value, 'utm_id=([^&]+)') is not null);;
  }
  ...
  dimension: UTM_SOURCE_Clicks {
    ...
    type: string
    sql:CASE
      WHEN (lower(${jobboard.name}) like lower(${traffic_source__source} )) and ${event_name}="sollicitatie_ontvangen2" THEN ${jobboard.name}
      WHEN (lower(${jobboard.name}) = lower(${traffic_source__source} )) and ${event_name}="sollicitatie_ontvangen2" THEN ${jobboard.name}
      END;;
  }
  ...
  dimension: campaign_name {
    ...
    sql: CASE
          ...
          WHEN lower(${traffic_source__name})=lower(${campaign.name}) and ${event_name}="sollicitatie_ontvangen2"
          THEN ${campaign.name}
  ...
  dimension: campaign_name_clicks {
    ...
    sql: CASE
                      ...
                      WHEN REGEXP_CONTAINS((lower(${traffic_source__name})), (lower(${campaign.name}))) = True and ${event_name}="sollicitatie_ontvangen2"
                      THEN ${campaign.name}
                      WHEN lower(${traffic_source__name})=lower(${campaign.name}) and ${event_name}="sollicitatie_ontvangen2"
                      THEN ${campaign.name}
                    END;;
  ...
  dimension: session_id{
    ...
    sql: (SELECT value.int_value
           FROM UNNEST(${event_params})
           WHERE event_name="sollicitatie_ontvangen2" AND key = 'ga_session_id');;
  ...
  dimension: rn_id{
    ...
    sql: (SELECT value.int_value
           FROM UNNEST(${event_params})
           WHERE event_name="sollicitatie_ontvangen2" AND key = 'rn_id');;
  ...
  dimension: vacancy_id {
    ...
    sql: (select lower((SELECT REGEXP_EXTRACT(value.string_value, 'vacancy=([^&]+)')
           FROM UNNEST(${event_params})
           WHERE event_name="sollicitatie_ontvangen2" AND key = 'page_referrer')));;
  }
  ...
  measure: sollicitatie {
    ...
    sql: CASE
          ...
          AND ${event_name}="sollicitatie_ontvangen2"
...
view: events_Zuijdher__items {
  ...
  dimension: events_20230505__items {
    type: string
    hidden: yes
    sql: events_20230505__items ;;
  }
...
view: events_Zuijdher__event_params {

  dimension: events_20230505__event_params {
    type: string
    hidden: yes
    sql: events_20230505__event_params ;;
  }
  dimension: key {
    type: string
    sql: key ;;
...
view: events_Zuijdher__user_properties {

  dimension: events_20230505__user_properties {
    type: string
    hidden: yes
    sql: events_20230505__user_properties ;;
  }
  dimension: key {
    type: string
    sql: key ;;
