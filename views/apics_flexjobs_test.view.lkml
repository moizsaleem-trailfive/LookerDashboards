view: apics_flexjobs_test {
  sql_table_name: `evident-catcher-381918.OnlineResultLegacyExcel.Apics Flexjobs_test`
    ;;
  ...
  dimension: is_selected_month {
    type: yesno
    sql: {% condition date_selector %} ${date_date} {% endcondition %} ;;
    }
  dimension: is_previous_month {
    type: yesno
    sql: ${date_date} > add_months(-1, {% date_start date_selector %}) AND ${date_date} < add_months(-1, {% date_end date_selector %}) ;;
    }
  measure: count_selected_month {
    type: count
    filters: { field: is_selected_month
    value: "yes" }
  }
  # measure: budget_change {
  #   type: number
  #   sql: ((${jobrapido_spend_incl} - offset(${jobrapido_spend_incl}, 1)) / offset(${jobrapido_spend_incl}, 1)) * 100 ;;
  #   value_format_name: percent_2f
  #   filters: {
  #     value: ""
  #   }
  # }
  ...
  measure: count_previous_month {
    type: count
    filters: { field: is_previous_month value: "yes" }
    }
